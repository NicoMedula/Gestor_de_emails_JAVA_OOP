...
public void sendEmail(EmailRequest request, StreamObserver<EmailResponse> responseObserver) {
    ...
    System.out.println("Destinatarios: " + request.getToList());
    ...
    for (String destinatarioEmail : request.getToList()) {
        ...
        Bandeja bandejaDestinatario = bandejas.computeIfAbsent(destinatarioEmail, k -> new Bandeja());
    ...
    EmailResponse response = EmailResponse.newBuilder()
            .setSuccess(true)
            .setMessage("Correo enviado exitosamente a " + request.getToList())
            .build();
